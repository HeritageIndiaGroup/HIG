...
const companyLinks = [
  ...
  { label: 'HIG - HERITAGE INDIA AGRO PRODUCER COMPANY LIMITED', href: '#agro' },
  { label: 'HIG - HERITAGE INDIA MICRO SERVICES FOUNDATION', href: '#micro' },
  { label: 'HIG â€“ HERITAGE MEDIA AND EDUCATION PRIVATE LIMITED', href: '#media' },
  { label: 'HIG - HERITAGE INDIA HOTEL & RESORT PRIVATE LIMITED', href: '#hotel' },
  { label: 'HIG - HERITAGE INDIA INFRA VENTURE PRIVATE LIMITED', href: '#infra' },
  { label: 'HIG - HERITAGE INDIA FILM PRODUCTION PRIVATE LIMITED', href: '#film' },
  { label: 'KashiKedarnath', href: '#kashi' },
...
const Nav = () => {
  ...
  useEffect(() => {
    // This effect handles smooth scrolling for internal links
    const internalLinks = document.querySelectorAll('a[href^="#"]');
    const handleClick = (e) => {
      const href = e.currentTarget.getAttribute('href');
      if (href && href.startsWith('#')) {
        ...
        if (el) {
          e.preventDefault();
          el.scrollIntoView({ behavior: 'smooth' });
          // Close mobile menu and dropdown after clicking an internal link
          setMobileOpen(false);
          setMobileDropdownOpen(false);
        }
      }
  ...
  return (
    ...
    >
      <div
        // Removed overflow-x-hidden from here
        className="max-w-screen-xl mx-auto px-6 py-3 flex items-center justify-between gap-4"
      >
        {/* Logo - visible only after scroll on both mobile & desktop */}
        {isSticky && (
          <a href="#home" className="flex items-center">
        ...
        {!isMobileScreen && (
          <nav className="flex items-center space-x-10 text-sm md:text-base font-semibold ml-auto">
            ...
            >
              ...
              {desktopDropdownOpen && (
                <div
                  className="absolute left-0 mt-2 w-full max-w-md bg-white text-black rounded-md shadow-lg z-50 p-2"
                  // Added a safety z-index here again, and ensured max-w-md for width control.
                  // 'w-full' here ensures it takes the full width of its 'relative' parent,
                  // but 'max-w-md' caps it. If 'w-full' is still problematic, remove it
                  // and rely only on 'max-w-md' or a fixed width.
                >
                  <ul className="py-2 text-sm font-normal max-h-[60vh] overflow-y-auto whitespace-normal break-words">
                    {companyLinks.map((item) => (
                      <li key={item.href}>
      ...
      {mobileOpen && isMobileScreen && (
        <div className="md:hidden bg-black px-6 py-6 text-white space-y-4 border-t border-gray-700">
          ...
          <button
            ...
            aria-controls="mobile-company-links"
          >
            <a href="#companies">Our Companies</a> {/* This 'a' tag here doesn't actually scroll, it's just text. */}
            <ChevronDown className={`w-4 h-4 transform transition-transform ${mobileDropdownOpen ? 'rotate-180' : ''}`} />
          </button>
              ...
              {companyLinks.map((item) => (
                <li key={item.href}>
                  <a
                    ...
                    className="block px-6 py-3 hover:text-[#F05A28] text-base"
                    onClick={() => {
                      setMobileOpen(false); // Close main mobile menu
